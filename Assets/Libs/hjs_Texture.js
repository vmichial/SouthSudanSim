...
var debug_tex_show_load=true; // DEBUG

var Texture=function(id, imgID, tileSize) { // 2 overloads
	/*
			Public vars
	*/
	this.id='';
	...
	*/
	...
	/*
			Public methods
	*/
	this.Texture=function(id, imgID, tileSize) { // Constructor; 1 overload
		if(id==undefined||imgID==undefined) {
			throw (this.id+': Constructor Texture(id, imgID, tileSize) No ID given'); // DEBUG
			return;
		}

		if(tileSize!=undefined) { // Texture(id, tileSize)
			if(typeof tileSize!='object') throw (this.id+': Texture(ctx, tileSize) tileSize must be an XY struct'); // DEBUG
		...
		this.__whatami='class Texture "'+id+'"';

		if(this.img!=null) { // Good image
			this.refreshProps();
		} else { // Error during load
			throw ('Image ID "'+imgID+'" not found, not initializing'); // DEBUG
		}
	}
	...
	this.refreshProps=function() {
		...
		if(this.tileSize.x!=this.size.x||this.tileSize.y!=this.size.y) {
			...
			if(debug_tex_show_load) console.log('Tex('+this.id+' :: '+this.img.id+') Loaded animated texture'); // DEBUG
		} else {
			this._frame=0;
			this._frameTime=0;
			this._frameCount=1;
			...
			if(debug_tex_show_load) console.log('Tex('+this.id+' :: '+this.img.id+') Loaded static texture'); // DEBUG
		}
	...
	// Draws to canvas
	this.draw=function(ctx, pos, scaleMul) {
		// DEBUG: Check canvas context
		if(ctx==undefined) throw (this.id+': draw(ctx, pos, scaleMul) context not passed'); // DEBUG
		if(this.id==null) throw (this.id+': sprite not initialized'); // DEBUG
		// DEBUG: Check position
		if(typeof pos!='object') throw (this.id+': draw(ctx, pos, scaleMul) pos must be an XY struct'); // DEBUG
		if(typeof pos.x!='number') throw (this.id+': draw(ctx, pos, scaleMul) parameter "pos.x" must be a number; got a typeof('+pos.x+')=='+typeof pos.x); // DEBUG
		if(typeof pos.y!='number') throw (this.id+': draw(ctx, pos, scaleMul) parameter "pos.y" must be a number; got a typeof('+pos.y+')=='+typeof pos.y); // DEBUG
		// DEBUG
		if(scaleMul!=undefined) { // DEBUG
			if(typeof scaleMul!='number') { // DEBUG
				if(typeof scaleMul!='object') throw (this.id+': draw(ctx, pos, scaleMul) scaleMul must be an XY struct or integer'); // DEBUG
				if(typeof scaleMul.x!='number') throw (this.id+': draw(ctx, pos, scaleMul) parameter "scaleMul.x" must be a number; got a typeof('+scaleMul.x+')=='+typeof scaleMul.x); // DEBUG
				if(typeof scaleMul.y!='number') throw (this.id+': draw(ctx, pos, scaleMul) parameter "scaleMul.y" must be a number; got a typeof('+scaleMul.y+')=='+typeof scaleMul.y); // DEBUG
			} // DEBUG
		} // DEBUG
		// DEBUG
		this._stepped=0; // DEBUG
		// DEBUG
		var scale=(scaleMul==undefined)?this.scale:vMul(this.scale, scaleMul);
		if(scale.x==0||scale.y==0) return;

		if(!this.hide) {
			var mx=(pos.x-this.mid.x*scale.x); // Maybe round pos?
			var my=(pos.y-this.mid.y*scale.y);
			if(this.static) {
				var size=vMul(this.size, scale);
				ctx.drawImage(
					this.img, // Image source
					mx, my,   // Position to place it
					size.x, size.y // Size to scale it to
				);
			} else {
				var size=vMul(this.tileSize, scale);
				ctx.drawImage(
					this.img, // Image source
	...
	this.face=function(angle) {
		if(typeof angle!='number') throw (this.id+': face(θ) parameter "θ" must be a number; got a typeof('+angle+')=='+typeof angle); // DEBUG
		// DEBUG
	...
	*/
	...
	// Constructor
	this.Texture(id, imgID, tileSize);
